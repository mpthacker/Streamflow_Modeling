...
usgs <- read.table("Gen_Data/USGS_Stream_Data/usgs_stream_gages_1990_2010_mmack.txt")
...
gage.monthly <- function(x,n){
  temp <- data.frame(stations[[n]][which(stations[[n]]$year_nu == years[x]), "mean_va"])
  dfl <- max(temp)/min(temp)
  sig2 <- var(temp)
  out<- c(dfl,sig2)
  names(out) <- c("DFL", "sig2")
  return(out)
}
...
write.table(dfl.mmack, "Gen_Data/calced_vars/dfl_mmack.txt")
write.table(sig2.mmack, "Gen_Data/calced_vars/sig2_mmack.txt")
